...
#Date: 2019-12-19
#Description: Evaluation script that calculates the Editdistance (with an external module).
#The script calculates the number of correct words and the total edit distance ignoring and taking into account the words case.
#On the Result ZIP file, an summary in Excel is created

import re
from io import BytesIO
import rrc_evaluation_funcs_1_1 as rrc_evaluation_funcs_1_1
import importlib
import xlsxwriter
import editdistance
...
def validate_data(gtFilePath, submFilePath,evaluationParams):
    ...
    gtFile = rrc_evaluation_funcs_1_1.decode_utf8(open(gtFilePath,'rb').read())
    if (gtFile is None) :
    ...
    submFile = rrc_evaluation_funcs_1_1.decode_utf8(open(submFilePath,'rb').read())
    if (submFile is None) :
...
def eval(predict,gt,output):
    args ={
        'o':output,
        'g':gt,
        's':predict
    }
    rrc_evaluation_funcs_1_1.main_evaluation(args,default_evaluation_params,validate_data,evaluate_method)

if __name__=='__main__':
    rrc_evaluation_funcs_1_1.main_evaluation(None,default_evaluation_params,validate_data,evaluate_method)